    ...
    <div class="card-body">
        <form action="{{ route('admin.borrowings.update', $borrowing->id) }}" method="POST">
            ...
            <div class="row">
                    ...
                    <div class="form-group">
                        ...
                        <select name="item_id" id="item_id" class="form-control" required onchange="updateItemInfo()">
                            ...
                            <option value="{{ $item->id }}" 
                                data-stock="{{ $item->stock + ($borrowing->item_id == $item->id ? $borrowing->qty : 0) }}" 
                                data-unit="{{ $item->unit->name }}"
                                {{ old('item_id', $borrowing->item_id) == $item->id ? 'selected' : '' }}>
                                {{ $item->name }} ({{ $item->code }}) - Stok: {{ $item->stock + ($borrowing->item_id == $item->id ? $borrowing->qty : 0) }} {{ $item->unit->name }}
                            </option>
                            @endforeach
                        </select>
                        @error('item_id')
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="number" name="qty" id="qty" class="form-control" value="{{ old('qty', $borrowing->qty) }}" required min="1" onchange="checkStock()">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" name="borrower_name" id="borrower_name" class="form-control" value="{{ old('borrower_name', $borrowing->borrower_name) }}" required placeholder="Nama lengkap peminjam">
                        @error('borrower_name')
                        <small class="text-danger">{{ $message }}</small>
                        @enderror
                    </div>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" name="borrower_phone" id="borrower_phone" class="form-control" value="{{ old('borrower_phone', $borrowing->borrower_phone) }}" placeholder="Nomor telepon">
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="date" name="return_date" id="return_date" class="form-control" value="{{ old('return_date', $borrowing->return_date->format('Y-m-d')) }}" required>
                        @error('return_date')
                        <small class="text-danger">{{ $message }}</small>
                        @enderror
                    </div>
            ...
            <div class="form-group">
                ...
                <textarea name="note" id="note" class="form-control" rows="3" placeholder="Catatan tambahan...">{{ old('note', $borrowing->note) }}</textarea>
            </div>
...
function updateItemInfo() {
    ...
    if (selectedOption.value) {
        ...
        stockInfo.textContent = `Stok tersedia (termasuk item ini): ${stock} ${unit}`;
    } else {
